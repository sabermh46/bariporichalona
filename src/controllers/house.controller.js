...
class HouseController {
        ...
        if (hasPermission && currentUser.id !== parseInt(ownerId)) {
          ...
          if (!isManaged) {
            return res.status(403).json({
              success: false,
              error:
                "You can only create houses for owners under your management",
            });
          }
        }
  ...
  async assignRenterToFlat(flatId, renterId) {
    return await db.transaction(async (trx) => {
      ...
      const flat = await trx("flat").where("id", flatId).first();
      ...
      await trx("flat").where("id", flatId).update({
        renterId: renterId,
        updatedAt: new Date(),
      });
  ...
  async removeRenterFromFlat(flatId) {
    return await db.transaction(async (trx) => {
      ...
      await trx("renter").where("id", flat.renterId).update({
        status: "inactive",
        updatedAt: new Date(),
      });
  ...
  async updateHouse(req, res) {
    try {
      const { id } = req.params;
      const { name, address, flatCount, metadata, active } = req.body;

      // Check if house exists
      const house = await db("house").where("id", id).first();
      ...
      // In updateData preparation:
        if (name !== undefined && allowedFields.name) {
      ...
      if (Object.keys(updateData).length === 0) {
        return res.status(400).json({
          success: false,
          error:
            "No valid fields to update or insufficient permissions for specified fields",
        });
      }

      updateData.updatedAt = new Date();

      await db("house").where("id", id).update(updateData);
    ...
    try {
      ...
      if (parseInt(caretakerCountResult.count) > 0) {
        return res.status(400).json({
          success: false,
          error:
            "Cannot delete house that has caretakers assigned. Remove caretakers first.",
        });
      }
      ...
      const owner = await db("user").where("id", house.ownerId).first();
    ...
    try {
        ...
        if (currentUser.role.slug === "house_owner") {
            ...
            if (!hasPermission) {
                return res.status(403).json({
                    success: false,
                    error: "You do not have permission to view house statistics",
                });
            }
        }

        // Get the house IDs that the user can access
        const allowedHouses = await houseQuery.clone().select("id");
        const allowedHouseIds = allowedHouses.map(h => h.id);
        ...
        const housesByMonth = await houseQuery
            .clone()
  ...
  async getHouseDetailsForPermission(houseId) {
    return await db("house").where("id", houseId).first();
  }
}
